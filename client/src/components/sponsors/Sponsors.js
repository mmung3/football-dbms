...
import {
  sponsorsFormHasErrors,
  generateSponsorFormWhereClause,
} from "../../utils/helpers";
...
const Sponsors = () => {
  ...
  const handleSubmit = () => {
    ...
    axios
      .post("http://localhost:65535/filter-sponsor", {
  ...
  return (
    <Container style={{ marginTop: "20px" }}>
      <Row>
        <h1>Sponsors</h1>
        <p>
          Find any sponsor by filtering below on the attributes you want. Or
          just press view with no filters to see all of the sponsors.
        </p>
      </Row>
      <Row>
        <Col>
        ...
        <Table striped bordered>
          ...
          <tbody>
            {sponsorTable.length !== 0 &&
              sponsorTable.map((d, i) => {
                return (
                  <tr key={`body-${i}`}>
                    {Object.keys(d).map((k, j) => {
                      return <td key={`cell-${j}`}>{d[k]}</td>;
                    })}
                  </tr>
                );
              })}
