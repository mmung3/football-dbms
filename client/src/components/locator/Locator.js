...
const Locator = () => {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:65535/venues")
      .then((res) => {
  ...
  const handleClick = () => {
    axios
      .post("http://localhost:65535/find-games/", { venueName: currVenueName })
      .then((res) => {
        setData(res.data);
      })
      .catch((err) => {
    ...
    <Container style={{ marginTop: "20px" }}>
      <Row>
        <Col>
          <h1>Game Locator</h1>
          <p>
            Find all games that have been played in any stadium using the
            dropdown below!
          </p>
        </Col>
      </Row>
                ...
                <Form.Group controlId="tables">
                  ...
                  >
                    ...
                    {venueNames.map((venueName, i) => {
                      return (
                        <option key={`table-${i}`} value={venueName}>
                          {venueName}
                        </option>
                      );
                    })}
