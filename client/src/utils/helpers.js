...
export const athleteFormHasErrors = async (athlete) => {
  ...
  if (/^0$/.test(athlete.height)) {
    ...
    errors.push(
      "Height is not a valid number, no symbol or letters are allowed",
    );
  }
  ...
  if (/^0$/.test(athlete.weight)) {
    ...
    errors.push(
      "Weight is not a valid number, no symbol or letters are allowed",
    );
  }
  ...
  if (!/^[1-9][0-9]{9}$/.test(athlete.phone_number)) {
    ...
    // is valid numeric wise but check in database
    try {
      const res = await axios.post("http://localhost:65535/phone-exists/", {
  ...
  if (!/^[0-9a-zA-Z._]+@([a-zA-Z]+\.)+[a-zA-Z]{2,4}$/.test(athlete.email)) {
    errors.push(
      "Invalid email format, only roman alphabet, numbers, . and _ are allowed.",
    );
  } else {
    // is valid format wise but check in database
    try {
      const response = await axios.post("http://localhost:65535/email-exists", {
        person_id: athlete.person_id,
        email: athlete.email,
      });
  ...
  //address
  if (
    !/^\d+(-\d+)?\s([A-Z][a-z]*\s)+(St|Street|Rd|Road|Dr|Drive|Lane|Way|Blvd|Boulevard|Ave|Avenue)[,.-]\s[A-Z][a-z]*$/.test(
      athlete.address,
    )
  ) {
    errors.push(
...
export const sponsorsFormHasErrors = (formData) => {
  ...
  if (!/^[1-9][0-9]*$|^$/.test(formData.moneyVal)) {
    errors.push(
      "Money is not a valid number, no symbol or letters are allowed",
    );
  }
