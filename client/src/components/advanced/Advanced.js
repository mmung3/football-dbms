...
const Advanced = () => {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:65535/tables")
      .then((res) => {
  ...
  useEffect(() => {
    if (currTable !== "") {
      axios
        .get(`http://localhost:65535/table/${currTable}/attributes`)
        .then((res) => {
  ...
  const handleAttributeChange = (e) => {
    ...
    if (selectedAttributes.includes(e.target.name)) {
      setSelectedAttributes(
        selectedAttributes.filter((s) => s !== e.target.name),
      );
    } else {
  ...
  const handleClick = (e) => {
    axios
      .post("http://localhost:65535/table", {
        table: currTable,
                  ...
                  <Form.Group controlId="tables">
                    ...
                    >
                      ...
                      {tables.map((table_name, i) => {
                        return (
                          <option key={`table-${i}`} value={table_name}>
                            {table_name}
                          </option>
                        );
                      })}
