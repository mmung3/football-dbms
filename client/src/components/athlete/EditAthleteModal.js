...
const EditAthleteModal = ({
  ...
  useEffect(() => {
    axios
      .get("http://localhost:65535/positions")
      .then((res) => {
        let ret = {};
  ...
  useEffect(() => {
    axios
      .get("http://localhost:65535/teams")
      .then((res) => {
  ...
  const editAthlete = async () => {
    ...
    if (errors.length === 0) {
      axios
        .put("http://localhost:65535/athlete", editedAthlete)
        .then((res) => {
          console.log(res);
        })
        .catch((err) => {
            ...
            <Col>
              <Form.Group controlId="jersey">
                ...
                <Form.Select
                  aria-label="jersey"
                  name="jersey_num"
                  value={athlete ? athlete.jersey_num : ""}
                  onChange={handleFormChange}
                >
                  <option value="default">-</option>
                  {Object.keys(positions).map((p, i) => {
                    return (
                      <option value={i + 1} key={`jersey-${i + 1}`}>
                        {i + 1} ({positions[i + 1]})
                      </option>
                    );
                  })}
            ...
            <Col>
              <Form.Group controlId="team">
                ...
                <Form.Select
                  aria-label="team"
                  name="current_team"
                  value={athlete ? athlete.current_team : ""}
                  onChange={handleFormChange}
                >
                  <option value="default">-</option>
                  {teams.map((t, i) => {
            ...
            <Col>
              <Form.Group controlId="start-date">
                ...
                <Form.Control
                  ...
                  name="date_started"
                  value={
                    athlete && athlete.date_started
                      ? athlete.date_started.split("T")[0]
                      : ""
                  }
                  onChange={handleFormChange}
                  required
            ...
            <Col>
              <Form.Group controlId="address">
                ...
                <Form.Control
                  ...
                  onChange={handleFormChange}
                  placeholder="1234 Main St, Vancouver"
                  required
                />
              </Form.Group>
            </Col>
            ...
            <Col>
              <Form.Group controlId="phone">
                ...
                <Form.Control
                  type="text"
                  name="phone_number"
                  value={athlete ? athlete.phone_number : ""}
                  onChange={handleFormChange}
                  required
                />
              </Form.Group>
