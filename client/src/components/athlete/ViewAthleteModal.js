...
const ViewAthleteModal = ({
  ...
  return (
    <Modal
      centered
      size="lg"
      show={showViewAthlete}
      onHide={() => setShowViewAthlete(false)}
    >
          ...
          <Row>
            <Col style={{ paddingLeft: "12%" }}>
              ...
              <p style={{ marginBottom: 0 }}>
                Date Started: {athlete?.date_started?.split("T")[0]}
              </p>
              <p style={{ marginBottom: 0 }}>
                Years of Experience:{" "}
                {athlete && athlete.date_started
                  ? new Date(
                      new Date() - new Date(athlete.date_started.split("T")[0]),
                    ).getFullYear() - 1970
                  : 0}
              </p>
              <p style={{ marginBottom: 0 }}>
                Jersey/Position: {athlete?.jersey_num} (
                {positions[athlete?.jersey_num]})
              </p>
              <p style={{ marginBottom: 0 }}>Team: {athlete?.current_team}</p>
              <p style={{ marginBottom: 0 }}>
                Awards: {awards === "" ? "N/A" : awards}
              </p>
            </Col>
            <Col>
              <p style={{ marginBottom: 0 }}>
              ...
              <p style={{ marginBottom: 0 }}>
                ...
                {athlete && athlete.birthdate
                  ? new Date(
                      new Date() - new Date(athlete.birthdate.split("T")[0]),
                    ).getFullYear() - 1970
                  : 0}
              </p>
              <p style={{ marginBottom: 0 }}>
                Phone Number: {athlete?.phone_number}
              </p>
